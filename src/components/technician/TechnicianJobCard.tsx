        ...
        <Link href={href} className="block p-5 pl-6">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              <p className="text-[12px] font-bold uppercase tracking-[0.16em] text-[#64748B]">#{shortOrderId(job.order_id).replace('UQ-', 'ORD-')}</p>
              <h3 className="mt-3 line-clamp-1 text-[18px] font-bold leading-6 tracking-normal text-[#000D32]">{serviceTitle}</h3>
            </div>
            <TechnicianStatusBadge status={visualStatus} />
          </div>
          <p className="mt-4 flex items-start gap-3 text-[14px] font-medium leading-6 text-[#64748B]">
            <MapPin className="mt-0.5 size-5 shrink-0 text-[#757680]" strokeWidth={2.2} aria-hidden="true" />
            <span className="line-clamp-1">{job.address}</span>
          </p>
          <p className="mt-5 flex items-start gap-3 border-t border-[#E0E3E5] pt-4 text-[14px] font-medium leading-6 text-[#64748B]">
  ...
  return (
    <TechnicianCard accent={status.accent} className="p-0">
      <div className={compact ? 'p-5 pl-6' : 'p-5 pl-6'}>
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <h3 className="line-clamp-2 text-[19px] font-bold leading-6 tracking-normal text-[#000D32]">{serviceTitle}</h3>
            <p className="mt-2 text-[15px] font-medium tracking-[0.04em] text-[#45464F]">{shortOrderId(job.order_id)}</p>
          </div>
          <TechnicianStatusBadge status={visualStatus} />
        </div>

        <div className="mt-6 grid gap-3 border-b border-[#E0E3E5] pb-6 text-[14px] font-medium leading-6 text-[#64748B]">
        ...
        <div className="mt-6 flex items-center justify-between gap-4">
          <p className="text-[21px] font-bold tracking-normal text-[#000D32]">{formatTaka(job.final_price)}</p>
          <Link
            href={href}
