...
export default async function TechnicianJobPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!job) {
    return (
      <TechnicianShell>
        <section className="mx-auto w-full max-w-[470px] rounded-[20px] border border-amber-200 bg-amber-50 p-6">
  ...
  return (
    <TechnicianShell>
      <div className="mx-auto w-full max-w-[470px] space-y-5">
        ...
        <aside className="h-fit rounded-[22px] border border-gray-100 bg-white p-6 shadow-sm">
          <h2 className="font-extrabold text-[#0B2A4A]">Update Status</h2>
          <div className="mt-4 grid gap-2">
            <a href={`tel:${job.customer_phone}`} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-gray-200 px-4 py-3 text-sm font-bold text-[#0B2A4A]">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center justify-center gap-2 rounded-2xl border border-gray-200 px-4 py-3 text-sm font-bold text-[#0B2A4A]"
            >
          ...
          {allowedActions.includes('accepted') ? (
            <form action={async () => { 'use server'; await updateBookingStatus(job.id, 'accepted'); }} className="mt-4">
              <SubmitButton pendingLabel="Accepting..." className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-[#2EA9D6] px-4 py-3 text-sm font-bold text-white">
          ...
          {allowedActions.includes('on_the_way') ? (
            <form action={async () => { 'use server'; await updateBookingStatus(job.id, 'on_the_way'); }} className="mt-3">
              <SubmitButton pendingLabel="Updating..." className="w-full rounded-2xl bg-[#0B2A4A] px-4 py-3 text-sm font-bold text-white">
                On The Way
              </SubmitButton>
            </form>
          ) : null}
          {allowedActions.includes('in_progress') ? (
            <form action={async () => { 'use server'; await updateBookingStatus(job.id, 'in_progress'); }} className="mt-3">
              <SubmitButton pendingLabel="Starting..." className="w-full rounded-2xl bg-[#0B2A4A] px-4 py-3 text-sm font-bold text-white">
                Start Work
              </SubmitButton>
            </form>
          ) : null}
          {allowedActions.includes('completed') ? (
            <form action={completeTechnicianJob} className="mt-4 space-y-3 rounded-[20px] border border-emerald-100 bg-emerald-50 p-3">
              ...
              <label className="block">
                ...
                <input
                  ...
                  className="min-h-[44px] w-full rounded-2xl border border-emerald-200 bg-white px-3 py-2 text-sm font-bold text-[#0B2A4A] outline-none focus:border-[#2EA9D6]"
                />
              </label>
              <label className="block">
                ...
                <textarea
                  ...
                  className="w-full rounded-2xl border border-emerald-200 bg-white px-3 py-2 text-sm font-semibold text-[#0B2A4A] outline-none focus:border-[#2EA9D6]"
                />
              </label>
              <SubmitButton
                pendingLabel="Completing..."
                confirmMessage="Mark this job as completed? This action cannot be undone."
                className="w-full rounded-2xl bg-[#25D366] px-4 py-3 text-sm font-bold text-white"
              >
                Complete Job
              </SubmitButton>
            </form>
          ...
          {allowedActions.length === 0 ? (
            <p className="mt-4 rounded-2xl bg-gray-50 p-4 text-sm font-semibold text-gray-500">No technician action is available for this status.</p>
          ) : null}
        </aside>
        ...
        <section className="grid gap-5">
          <div className="rounded-[22px] border border-gray-100 bg-white p-6 shadow-sm">
            <div className="flex items-center gap-3">
              <div className="flex size-10 items-center justify-center rounded-2xl bg-[#F0F9FC] text-[#2EA9D6]">
            ...
            <form action={rescheduleTechnicianJob} className="mt-5 grid gap-4 sm:grid-cols-2">
              ...
              <label className="block">
                ...
                <input
                  ...
                  className="min-h-[46px] w-full rounded-2xl border border-gray-200 bg-gray-50 px-3 py-2 text-sm font-bold text-[#0B2A4A] outline-none focus:border-[#2EA9D6] focus:bg-white"
                />
              </label>
              <label className="block">
              ...
              <label className="block sm:col-span-2">
                ...
                <textarea
                  ...
                  className="w-full rounded-2xl border border-gray-200 bg-gray-50 px-3 py-2 text-sm font-semibold text-[#0B2A4A] outline-none focus:border-[#2EA9D6] focus:bg-white"
                />
              </label>
              <div className="sm:col-span-2">
                <SubmitButton pendingLabel="Saving schedule..." className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-[#2EA9D6] px-4 py-3 text-sm font-bold text-white">
          ...
          <div className="rounded-[22px] border border-gray-100 bg-white p-6 shadow-sm">
            ...
            <form action={addTechnicianJobNote} className="mt-5 space-y-4">
              ...
              <SubmitButton pendingLabel="Adding note..." className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-[#0B2A4A] px-4 py-3 text-sm font-bold text-white">
...
function Info({
  ...
  return (
    <div className={`rounded-2xl border border-gray-100 bg-gray-50 p-4 ${wide ? 'sm:col-span-2' : ''}`}>
      <p className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-gray-400">
