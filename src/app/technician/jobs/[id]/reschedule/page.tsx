    ...
    <TechnicianShell title="Reschedule Service" backHref={`/technician/jobs/${job.order_id}`} hideNav wide largeHeader titleAlign="left">
      <TechnicianCard accent={status.accent} className="mt-1 p-5 pl-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-[13px] font-medium uppercase tracking-[0.22em] text-[#45464F]">Order ID</p>
            <h1 className="mt-5 text-[24px] font-semibold leading-8 text-[#191C1E]">{shortOrderId(job.order_id)}</h1>
            <p className="mt-4 flex items-center gap-3 text-[18px] font-medium leading-7 text-[#45464F]">
      ...
      <form action={rescheduleTechnicianJob} className="mt-8 pb-28">
        ...
        <section>
          <h2 className="text-[22px] font-semibold leading-7 text-[#191C1E]">Select New Date</h2>
          <div className="mt-4 grid grid-cols-5 gap-2">
            {days.map((day, index) => (
              <label key={day.value} className="cursor-pointer">
                ...
                <span className="flex min-h-[80px] flex-col items-center justify-center rounded-lg bg-white text-[#191C1E] shadow-[0_1px_2px_rgba(18,35,77,0.04)] ring-1 ring-[#D8DADC] transition peer-checked:bg-[#000D32] peer-checked:text-white">
                  <span className="text-[12px] font-semibold uppercase tracking-[0.14em] text-inherit">{day.weekday}</span>
                  <span className="mt-3 text-[24px] font-semibold leading-none">{day.day}</span>
                </span>
              </label>
            ))}
          </div>
        ...
        <section className="mt-8">
          <h2 className="text-[22px] font-semibold leading-7 text-[#191C1E]">Available Time</h2>
          <div className="mt-4 grid grid-cols-2 gap-3">
            {times.map((time, index) => (
              <label key={time} className="cursor-pointer">
                <input className="peer sr-only" type="radio" name="preferredTime" value={time} defaultChecked={index === 1} required />
                <span className="flex min-h-[58px] items-center justify-center rounded-lg bg-white text-[17px] font-medium tracking-[0.04em] text-[#191C1E] shadow-[0_1px_2px_rgba(18,35,77,0.04)] ring-1 ring-[#D8DADC] transition peer-checked:bg-[#000D32] peer-checked:text-white">
                  {time}
                </span>
              </label>
            ))}
        ...
        <section className="mt-8">
          <h2 className="text-[22px] font-semibold leading-7 text-[#191C1E]">Reason for Reschedule</h2>
          <select
            name="rescheduleReason"
            required
            defaultValue=""
            className="mt-4 min-h-[58px] w-full rounded border border-[#C5C6D0] bg-[#F2F4F6] px-5 text-[17px] font-medium text-[#191C1E] outline-none focus:border-[#000D32]"
          >
          ...
          <textarea
            ...
            className="mt-4 min-h-[140px] w-full rounded border border-[#C5C6D0] bg-[#F2F4F6] p-5 text-[17px] font-medium leading-7 text-[#191C1E] outline-none placeholder:text-[#45464F] focus:border-[#000D32]"
          />
        </section>
        ...
        <div className="fixed bottom-0 left-1/2 z-40 w-full max-w-[430px] -translate-x-1/2 bg-white/90 px-4 py-4 shadow-[0_-16px_30px_rgba(18,35,77,0.05)] backdrop-blur">
          <div>
            <SubmitButton pendingLabel="Saving..." className="inline-flex min-h-[58px] w-full items-center justify-center rounded bg-[#000D32] text-[19px] font-semibold text-white shadow-[0_8px_18px_rgba(0,13,50,0.18)]">
          Confirm Reschedule
            </SubmitButton>
          </div>
        </div>
